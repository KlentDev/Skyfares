...
**Important:** always pass `--config wrangler.toml` explicitly, as good practice. The specific
footgun this used to guard against — a stray repo-root `wrangler.jsonc`, auto-generated by the
Cloudflare GitHub App, that bare `wrangler deploy` would resolve instead of this folder's own
config — was removed on 2026-07-24 (see krisflyer.md). It shared the `skyfares-altitude` Worker
name and would have deployed the entire static site as Workers Assets, failing on `.git` being
over the 25 MiB asset size limit.
